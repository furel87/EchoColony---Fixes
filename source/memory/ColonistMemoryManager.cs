...
namespace EchoColony
{
    public class ColonistMemoryManager : GameComponent
    {
        ...
        public override void GameComponentTick()
        {
            ...
            // Se evalúa cada 1000 ticks (~16 segundos reales a velocidad x1)
            if (Find.TickManager.TicksGame % 1000 == 0)
            {
                ...
                if (currentDay > lastRecordedDay)
                {
                    ...
                    foreach (var kvp in memoryPerPawn)
                    {
                        ...
                        // Inicia una corrutina independiente por colono
                        if (MyStoryModComponent.Instance != null)
                        {
        ...
        /// <summary>
        /// Secuencia individual por colono: espera su memoria de turnos y luego procesa su fin de día.
        /// </summary>
        private IEnumerator ProcessPawnEndOfDaySequence(Pawn pawn, ColonistMemoryTracker tracker, int dayToProcess)
        {
            // 1. Espera a que la API responda y guarde los turnos pendientes del colono (umbral = 1)
            yield return ColonistMemoryHelper.CheckAndGenerateMemoryRoutine(pawn, 1);

            // 2. Con la memoria de turnos al día, consolida el resumen del día
            try
            {
